...
class GroupConfigurationsSearch(CourseTestCase, MixedWithOptionsTestCase):
    ...
    def setUp(self):
        ...
        self.expected_grouped_result = [
            call(
                'courseware_content',
                {
                    'start_date': datetime(2015, 4, 1, 0, 0, tzinfo=tzutc()),
                    'content': {'html_content': '', 'display_name': unicode(self.html_unit.display_name)},
                    'course': unicode(self.course.id),
                    'content_type': 'Text',
                    'content_groups': [1],
                    'id': unicode(self.html_unit.location)
                }
            ),
            call(
                'courseware_content',
                {
                    'start_date': datetime(2015, 4, 1, 0, 0, tzinfo=tzutc()),
                    'content': {'display_name': unicode(self.vertical.display_name)},
                    'course': unicode(self.course.id),
                    'content_type': 'Sequence',
                    'content_groups': [1],
                    'id': unicode(self.vertical.location)
                }
            ),
            call(
                'courseware_content',
                {
                    'content': {'display_name': unicode(self.sequential.display_name)},
                    'course': unicode(self.course.id),
                    'id': unicode(self.sequential.location),
                    'content_type': 'Sequence',
                    'start_date': datetime(2015, 3, 1, 0, 0, tzinfo=tzutc())
                }
            ),
            call(
                'courseware_content',
                {
                    'content': {'display_name': unicode(self.chapter.display_name)},
                    'course': unicode(self.course.id),
                    'id': unicode(self.chapter.location),
                    'content_type': 'Sequence',
                    'start_date': datetime(2015, 3, 1, 0, 0, tzinfo=tzutc())
                }
            )
    ...
    def test_content_group_gets_indexed(self):
        ...
        self.assertEqual(added_to_index, 4)
        response = searcher.search(field_dictionary={"course": unicode(self.course.id)})
        self.assertEqual(response["total"], 4)
        ...
        self.client.ajax_post(
            reverse_usage_url("xblock_handler", self.html_unit.location),
        ...
        with patch(settings.SEARCH_ENGINE + '.index') as mock_index:
            ...
            self.assertEqual(self.expected_grouped_result, mock_index.mock_calls)
    ...
    def test_content_group_gets_inherited_from_parent(self):
        ...
        with patch(settings.SEARCH_ENGINE + '.index') as mock_index:
            self.reindex_course(self.store)
            self.assertTrue(mock_index.called)
            self.assertEqual(self.html_group_result, mock_index.mock_calls[0])

    def test_content_group_not_indexed(self):
        """ indexing course without content groups added test """

        with patch(settings.SEARCH_ENGINE + '.index') as mock_index:
            self.reindex_course(self.store)
            self.assertTrue(mock_index.called)
            self.assertEqual(self.html_nogroup_result, mock_index.mock_calls[0])
